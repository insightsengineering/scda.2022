...
#'
synthetic_cdisc_data <- function(name, external = FALSE) {

  avail <- ls_synthetic_cdisc_data(external = external)
...
#'
#' ls_synthetic_cdisc_data(external = TRUE)
ls_synthetic_cdisc_data <- function(external = FALSE) {

  pkgs <- if (external) {
    all_pkgs <- as.vector(installed.packages()[, "Package"])
    unique(all_pkgs[grepl("^scda\\.[[:digit:]]{4}$", all_pkgs)])
  } else {
    "scda.2021"
  }
